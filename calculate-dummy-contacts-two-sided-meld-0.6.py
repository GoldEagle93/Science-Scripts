...
for i in N1s:
    ...
    for j in CAs:
        ...
        if dist < threshold*.1:
            ...
            collection.append(str(last_res+c) + ' S0 SDM ' + str(topology.atom(j).residue.index+1) + ' ' + topology.atom(j).name + ' ' + str(topology.atom(j).residue.name) + ' ' + str(threshold))
    ...
    if len(collection)>3 and topology.atom(i).residue.index+1 in left:
        ...
        if n == 0:
            projector.append(str(topology.atom(i).residue.index+1) + ' N1 ' + topology.atom(i).residue.name + ' ' + str(last_res+c) + ' S0 SDM')
            n += 1
        c += 1
