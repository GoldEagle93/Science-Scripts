...
def plot_type(wt,output,output2):
    ...
    for i in f:
        ...
        i=i.split()
        good.append(i)
        G.add_edge(int(i[0]),int(i[2]),color='b',weight=wt,edgeprops={'alpha':100})

    f=open('good_2.dat','r').readlines()
    for i in f:
        i=i.strip()
        i=i.split()
        good.append(i)
        G.add_edge(int(i[0]),int(i[2]),color='r',weight=wt,edgeprops={'alpha':100})

    #f=open('native.dat','r').readlines()
    #for i in f:
        #i=i.strip()
        #i=i.split()
        #if i not in good:
...
plot_type(5,'true_new.pdf','true_new.png')
#plot_type(0,'satisfied_new.pdf','satisfied_new.png')
