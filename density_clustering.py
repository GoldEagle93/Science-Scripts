...
def feature(traj_loc, pdb_loc, traj_start=traj_start, traj_end=traj_end, sieve_traj=sieve_traj, 
    ...
    traj = md.load_dcd(traj_loc[0],top=pdb_loc)[traj_start[0]:traj_end[0]]    
    if num_traj>1:
...
def main():
    ...
    np.save('colors',mirror_binary_simi[int(max_g_simi)])
    plt.figure()
    ...
    plt.savefig('density_rank.png', bbox_inches='tight', dpi=300) 
    print('-----save fig-----')
