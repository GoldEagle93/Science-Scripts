...
bold.reverse()
count = 0
reversed_columns = list(range(1, len(df.columns)))#.reverse()
reversed_columns.reverse()
for traj in reversed_columns:
    plt.plot(df[df.columns[0]], df[df.columns[traj]], label = traj, alpha=0.8, marker='.', linestyle='None', color = bold[count], markersize=4)
    count += 1
...
plt.ylim(0, 100)
plt.savefig('rmsd30.png', dpi=300, bbox_inches='tight')        
